...
function normalizeNumber (number) {
    ...
    const value = parseInt(number).toLocaleString('ru-RU', options)
...
fetch('russia_with_data.json').then(response => {
        ...
        this._div.innerHTML = '<h4>Статистика по регионам</h4></br>' + (props
            ...
            `<table>
                <tr>
                    ...
                    <td>${normalizeNumber(props.man_mean)}</td>
                </tr>
                <tr>
                    <td>Средний доход чиновников-женщин</td>
                    <td>${normalizeNumber(props.fem_mean)}</td>
                </tr>
                <tr>
                    <td>Средний доход в регионе</td>
                    <td>${normalizeNumber(props.salary_mean)}</td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td>${normalizeNumber(props.savings_mean)}</td>
                </tr>
                <tr>
                    <td>Средний доход супругов чиновников</td>
                    <td>${normalizeNumber(props.siblings_mean_income)}</td>
                </tr>
                <tr>
    ...
    L.control.layers({
        'Отношение дохода чиновника к среднему': ratioLayer,
        'Индекс роста региона': growthLayer
    }, {}).addTo(map)
    map.invalidateSize()
})
